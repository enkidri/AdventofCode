...
int CalcRating(Grid &grid, Index2D trail_head) {
  stack<Index2D> visited_locations;
  vector<Height> goal_locations;
  ...
  while (!visited_locations.empty()) {
    ...
    if (grid[current_location.j][current_location.i].height == 9) {
      ...
      mem[current_location.j][current_location.i].height++;
      continue;
    }
  ...
  int rating = 0;
  for (auto &trail_head : goal_locations) {
    rating += mem[trail_head.current.j][trail_head.current.i].height;
  }
  return rating;
}
...
void partTwo(char *file_path) {
  ...
  // Perform the search
  int answer = 0;
  for (size_t idx = 0; idx < trail_heads.size(); idx++) {
    // Add previously added locations
    answer += CalcRating(grid, trail_heads[idx]);
  }
